...
function isCreditCardNumber(ccn) {
  ...
  arr.forEach((num, index) => {
    arr[index] = Number.parseInt(num, 10);
    if ((index + 1) % 2 === 0) {
      arr[index] *= 2;
    }

    if (arr[index] > 9) {
      const numArr = arr[index].toString().split('');
      arr[index] = Number.parseInt(numArr[0], 10) + Number.parseInt(numArr[1], 10);
    }
  });
...
function getDigitalRoot(num) {
  ...
  let n = num;
  n = String(n).split('').map((digit) => Number(digit));
  n = n.reduce((acc, p) => acc + p);
...
function isBracketsBalanced(str) {
  ...
  strArr.forEach((bracket) => {
    if (brackets[bracket]) {
      stack.push(bracket);
    } else if (brackets[stack[stack.length - 1]] === bracket) {
      stack.pop();
    } else {
      stack.push(bracket);
    }
...
function toNaryString(num, n) {
  return Number.parseInt(num.toString(n), 10);
}
...
function getMatrixProduct(m1, m2) {
  ...
  if (m1Cols !== m2Rows) {
    throw new Error('Matrixes cannot be multiplied');
  }
